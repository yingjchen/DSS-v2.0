...
HEATMAP_SD <- function(df, proportion = 1, filename = ""){
  ...
  if (nrow(df) > 1 & ceiling(ncol(df)*proportion) > 1){
    ...
    if (filename == "") {ggsave(paste0("./example_step8_Breeze_DSS_", ceiling(ncol(df)*proportion), "drugs_heatmap.pdf"), p0, height = 10,width = 10)}else
    ...
    if (filename == "") {ggsave(paste0("./example_step8_Breeze_DSS_", ncol(df), "drugs_heatmap.pdf"), p0, height = 10,width = 10)}else
    {ggsave(filename, p0, height = 10,width = 10)}
    ...
    if (filename == "") {ggsave("./example_step8_Breeze_DSS_barplot.pdf", p0, height = 10, width = 10)}else
    {ggsave(filename, p0, height = 10, width = 10)}
...
CHEMO_TAREGTED_PLOT <- function(df, metric, filename = ""){
  ...
  if (filename == "") {ggsave(paste0("./example_step13_chemotherapy_taregted_drugs_", metric, "_distribution.pdf"), p3, height = 10, width = 10)}else
...
SELECTIVE_SCORE_PLOT <- function(df, filename = ""){
  ...
  if (filename == "") {ggsave("./example_step13_density_distribution_response_scores.pdf", p1, height = 10, width = 10)}else
